...
class BaseGraphicalLasso(object):
    ...
    def correct_edges(self):
        ...
        for real_network, network in zip(self.real_thetas, self.thetas):
            current_real_edges = 0
            current_correct_positives = 0
            current_all_positives = 0
            for i in range(self.dimension - 1):
                for j in range(i + 1, self.dimension):
                    if real_network[i, j] != 0:
                        current_real_edges += 1
                        self.real_edges += 1
                        if network[i, j] != 0:
                            current_correct_positives += 1
                            current_all_positives += 1
            ...
            if current_all_positives:
                current_precision = float(current_correct_positives)/float(current_all_positives)
                current_recall = float(current_correct_positives)/float(current_real_edges)
                self.microf1.append(2*(current_precision*current_recall)/(current_precision + current_recall))

        if self.all_positives:
            ...
            self.f1micro = np.mean(np.array(self.microf1))
    ...
    def print_matrix(self,thenum):
        print "\nNetwork " + str(thenum) + ":"
        for j in range(self.dimension):
            print self.thetas[thenum][j, :]

    def get_ready_draw(self, thenum):
        if 'case_study' in self.filename:
            num = thenum
            with open('result_draw.json','w') as wf:
                wf.write('[\n')
                for i in range(self.dimension):
                    dic = OrderedDict()
                    dic['name'] = self.entity_list[i]
                    depends = []
                    val = []
                    for j in range(self.dimension):
                        if i != j and abs(self.thetas[num][i][j]) > 0.1:
                            depends.append(self.entity_list[j])
                            val.append(self.thetas[num][i][j])
                    dic['depends'] = depends
                    dic['val'] = val      
